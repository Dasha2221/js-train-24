...
function task6() {
  ...
  const promise = new Promise((resolve, reject) => {
    setImmediate(() => {
      ...
      if (randomValue < 0.5) {
        resolve(`Більше ніж 0.5. Згенероване число: ${randomValue}`);
      } else {
        // Інакше відхиляемо проміс з рядком "Менше ніж 0.5".
        reject(`Менше ніж 0.5. Згенероване число: ${randomValue}`);
      }
    });
  });

  // Використовуємо then для обробки зарезолвленого проміса та виводимо результат.
  promise
    .then((value) => {
      console.log(`Проміс зарезолвився з значенням: ${value}`);
    })
    // Обробляємо помилку за допомогою catch та виводимо її.
    .catch((error) => {
