...
function task9() {
  ...
  fetchWithError()
    .then((data) => {
      console.log("Дані з fetchWithError:", data); // Ця гілка не буде виконана через помилку
    })
    .catch((error) => {
      console.error("Помилка в fetchWithError:", error); // Обробка помилки та виведення її в консоль
    });
